...
bool bInit = false;
bool bAngle = false;
double dMAM_Angle_pie = 0;
ros::Time last_time, current_time;

geometry_msgs::Quaternion Quaternion_robot_camera(double &mam_Angle_pie){
  tf::Quaternion q_init_gazebo = tf::createQuaternionFromRPY(0, pitch*M_PI/180.0, yaw+mam_Angle_pie);
...
gazebo_msgs::SetModelState baselink_to_camerabody_to_gazebo( gazebo_msgs::SetModelState& objstate, double &mam_Angle_pie){
  double pitch = 28.0;
...
int main(int argc, char** argv){
  ...
  while(n.ok() /* && bInit && bVel */ ){
    ...
    if( bInit && bVel ){
      ...
      // 5.1 (3) 使用cmdvel更新位姿.footprint__pose是更新后的机器人底盘位姿
      ...
      objstate = baselink_to_camerabody_to_gazebo(objstate, dMAM_Angle_pie);
      client.call(objstate);
      ...
      camera_trans.transform.translation.z = height;
      geometry_msgs::Quaternion q_robot_camera ;   //机器人到相机坐标系的旋转关系
      camera_trans.transform.rotation = objstate.request.model_state.pose.orientation;
